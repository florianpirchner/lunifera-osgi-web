...
public class EnvironmentAdaptingServlet implements PseudoServlet {
	private static Logger log = Logger
			.getLogger(EnvironmentAdaptingServlet.class.getName());
	private PseudoServlet servlet;
	private Server server;

	public EnvironmentAdaptingServlet(final Server server,
			final Configuration configuration) {
		this.server = server;
		// if (configuration.getAttributeAsBoolean("useAsyncContext",
		// isAsyncARPAvailable())) {
		// log.log(Level.INFO,
		// "Adapting to Servlet 3.0 AsyncContext environment");
	...
	private boolean isAsyncARPAvailable() {
		try {
			this.getClass().getClassLoader()
					.loadClass("javax.servlet.AsyncContext");
			return true;
