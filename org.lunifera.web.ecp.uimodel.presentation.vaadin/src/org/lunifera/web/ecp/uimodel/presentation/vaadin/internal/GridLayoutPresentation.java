...
public class GridLayoutPresentation extends AbstractLayoutPresenter {
	...
	private static class Row {
		...
		public void addCell(int column, Cell cell) {
			// if empty cells between maxIndex and column, then fill with empty
			// cells
			if (column > cells.size()) {
				for (int i = cells.size(); i < column; i++) {
					cells.add(i, new EmptyCell());
				}
			}
	...
	private static class Column {
		...
		public void addCell(int row, Cell cell) {
			...
			// if empty cells between maxIndex and row, then fill with empty
			// cells
			if (row > cells.size()) {
				for (int i = cells.size(); i < row; i++) {
					cells.add(i, new EmptyCell());
				}
			}
			int maxIndex = cells.size() - 1;
